...
class Rex_Admin {
	...
	public function enqueue_admin_styles() {
		...
		if ( $this->plugin_screen_hook_suffix == $screen->id ) {
			wp_enqueue_style( $this->plugin_slug .'-admin-styles', plugins_url( 'assets/css/admin.css', __FILE__ ), array(), Rex::VERSION );
		}

		if ( $this->plugin_slug == $screen->post_type ) {
			wp_enqueue_style( $this->plugin_slug .'-admin-meta-boxes-styles', plugins_url( 'assets/css/admin-meta-boxes.css', __FILE__ ), array(), Rex::VERSION );
			wp_enqueue_style( $this->plugin_slug .'-admin-select2-styles', plugins_url( 'assets/css/select2.css', __FILE__ ), array(), Rex::VERSION );
		}
	...
	public function enqueue_admin_scripts() {
		...
		if ( isset( $this->plugin_screen_hook_suffix ) ) {
			if ( $this->plugin_screen_hook_suffix == $screen->id ) {
				wp_enqueue_script( $this->plugin_slug . '-admin-script', plugins_url( 'assets/js/admin.js', __FILE__ ), array( 'jquery' ), Rex::VERSION );
			}
		}
		...
		// 			'add' == $screen->action ) {
		if ( $this->plugin_slug == $screen->post_type ) {
			wp_enqueue_script( $this->plugin_slug . '-admin-meta-boxes-script', plugins_url( 'assets/js/admin-meta-boxes.js', __FILE__ ), array( 'jquery' ), Rex::VERSION );
			wp_enqueue_script( $this->plugin_slug . '-admin-meta-boxes-select2-script', plugins_url( 'assets/js/select2.min.js', __FILE__ ), array( 'jquery' ), Rex::VERSION );
		}
	...
	public function add_plugin_admin_menu() {
		...
		$this->plugin_screen_hook_suffix = add_options_page(
			__( 'Book Recommendations Settings', $this->plugin_slug ),
			__( 'Book Recommendations', $this->plugin_slug ),
			'manage_options',
			$this->plugin_slug,
